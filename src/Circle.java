...
public class Circle extends ClosedShape {
    ...
    /**
     * It makes the circle to pulse on the screen.
     */
    public void pulsing() {
    	if(isPulsing) {
    		if(diameter + ClosedShape.pulseRate > this.max) {
    			this.diameter = this.max;
    			ClosedShape.pulseRate = -1;
    		} else if(diameter + pulseRate < this.min){
    			this.diameter = this.min;
    			ClosedShape.pulseRate = 1;
    		}	
    		this.diameter += ClosedShape.pulseRate;
    	}
